DROP 
  TABLE IF EXISTS public.shipping_info;
...
FROM 
  ...
  LEFT JOIN (
    SELECT 
      id AS shipping_transfer_id, 
      concat(
        transfer_type, ':', transfer_model
      ) AS shipping_transfer_description 
    FROM 
      public.shipping_transfer
  ) AS st ON s.shipping_transfer_description = st.shipping_transfer_description 
  LEFT JOIN (
    SELECT 
      agreement_id AS shipping_agreement_id, 
      concat(
        agreement_id, ':', agreement_number, 
        ':', agreement_rate, ':', agreement_commission
      ) AS vendor_agreement_description 
    FROM 
      public.shipping_agreement
  ) AS sa ON s.vendor_agreement_description = sa.vendor_agreement_description 
  LEFT JOIN (
    SELECT 
      id AS shipping_country_rate_id, 
      shipping_country 
    FROM 
