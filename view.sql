...
a AS (
    SELECT 	b.shipping_id, 
           	(CASE WHEN b.shipping_end_fact_datetime > b.shipping_plan_datetime THEN '1' ELSE '0' END) AS is_delay,
  			(CASE WHEN b.status = 'finished' THEN '1' ELSE '0' END) AS is_shipping_finish,
  			(CASE WHEN b.shipping_end_fact_datetime > b.shipping_plan_datetime THEN (b.shipping_end_fact_datetime-b.shipping_plan_datetime) ELSE '0' END) AS delay_day_at_shipping 
    FROM (SELECT 	ss.shipping_id,
		  			ss.shipping_end_fact_datetime,
		  			s.shipping_plan_datetime,
		  			ss.status
		  FROM public.shipping_status ss
		 LEFT JOIN public.shipping s ON ss.shipping_id=s.shippingid) AS b
),
c AS (
    SELECT 	d.shipping_id,
  			(d.payment_amount *(d.shipping_country_base_rate + d.agreement_rate + d.shipping_transfer_rate)) AS vat,
           	(d.payment_amount * d.agreement_commission) AS profit
    FROM (SELECT 	si.shipping_id,
		  			si.payment_amount,
...
SELECT 	DISTINCT si.shipping_id,
 		si.vendor_id,
 		st.transfer_type,
